...
function parse_phrase(user_text){
	...
	// 	replies.push('Hi there! Try asking me about a certain course like "When is COMP 202" or using a course title like "who teaches forest management?"')
	// 	return replies;
	// }
	...
	if (course_matches && course_matches.length > 0){
		...
		for (var j = 0; j < course_matches.length; j++) {
			...
			parsed.push(subject+' '+code);
			queries = queries.concat( search.generate_query_by_subject_code(subject, code, year) );
		};
		...
		parsed.push(title);
		if(title.length>0){
			queries = queries.concat( search.generate_query_by_title(title, year) );
		}
	}

	return [parsed, queries]
	// this should be dealt with outside
	// if(queries.length === 0){
	// 	replies.push('Try asking me about a certain course like "When is COMP 202" or using a course title like "who teaches forest management?"');
	// 	replies.push('I didn\'t quite understand what you\'re trying to ask me...');
	// }
